...
- **스트릭 계산**: `lastDate !== today`일 때만 1회 평가. 전날 모든 항목 100% 완료 시 +1, 미완료 또는 하루 이상 건너뛰면 0 리셋. **같은 날이면 계산 자체를 건너뜀** (수동 수정 보호).
- **수동 수정**: streak 카드의 연필 아이콘으로 앱 내에서 직접 수정 가능. 다음 자정(날짜 변경) 전까지 자동 계산에 의해 덮어씌워지지 않음.
